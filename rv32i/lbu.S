# Test for the LBU (Load Byte Unsigned) instruction.
...
MEM_TEST_REGION:
    ...
    .byte 0x80          # Byte 4 (offset 4) = Min negative signed byte (as signed)
...
_start:
    # --- Test Cases for LBU ---
    # LBU loads an 8-bit byte from memory: rd = mem[rs1 + imm]
    # The loaded byte is zero-extended to XLEN (32-bit for RV32).
    ...
    # Test case 2: Load byte from offset 1 (value 0xFE, treated as unsigned)
    # t2 = MEM_TEST_REGION[1] = 0x000000FE
    lbu     t2,     1(t0)
    ...
    # Test case 4: Load byte from offset 3 (max positive signed, also positive unsigned)
    # t4 = MEM_TEST_REGION[3] = 0x7F
    lbu     t4,     3(t0)

    # Test case 5: Load byte from offset 4 (value 0x80, treated as unsigned)
    # t5 = MEM_TEST_REGION[4] = 0x00000080
    lbu     t5,     4(t0)

    # Test case 6: Load byte from offset 5
    # t6 = MEM_TEST_REGION[5] = 0x000000CD
    lbu     t6,     5(t0)
...
GPR07_FINAL_VALUE: .dword 0x000000FE    # t2 (x7)
GPR28_FINAL_VALUE: .dword 0x00000000    # t3 (x28)
GPR29_FINAL_VALUE: .dword 0x0000007F    # t4 (x29)
GPR30_FINAL_VALUE: .dword 0x00000080    # t5 (x30)
GPR31_FINAL_VALUE: .dword 0x000000CD    # t6 (x31)
